#ifndef LUMINA_WEB_SERVER_H
#define LUMINA_WEB_SERVER_H
...
// Instancia global del servidor para acceso desde lambdas
class LuminaWebServer;
static LuminaWebServer* g_webServer = nullptr;

/**
 * @class LuminaWebServer
 * @brief Servidor web para configuración y control del sistema (ESP32)
 */
class LuminaWebServer {
private:
  WebServer server;
  ...
  String generateConfigurationPage() const {
    if (!phaseManager) return "<h1>Error: PhaseManager not initialized</h1>";
  ...
  /**
   * @brief Manejador para la página raíz
   */
  void handleRoot() {
...
public:
  LuminaWebServer() : server(80), initialized(false), phaseManager(nullptr) {
    g_webServer = this;  // Guardar referencia global
  }
  ...
  void begin(PhaseManager* pm) {
    ...
    server.on("/save_config", [this]() { this->handleSaveConfig(); });

    // Rutas de prueba del portal cautivo
    server.on("/generate_204", [this]() { this->handleRoot(); });
    server.on("/fwlink", [this]() { this->handleRoot(); });
    server.on("/hotspot-detect.html", [this]() { this->handleRoot(); });
